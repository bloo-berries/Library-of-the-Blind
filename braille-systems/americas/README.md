# Americas Braille Systems

## North America

### [Unified English Braille (UEB)](https://en.wikipedia.org/wiki/Unified_English_Braille)
- **Languages Supported:** English (US, Canada)
- **Key Characteristics:** Single unified code for literary and technical material
- **Status:** Current standard

### [American Braille (AB) / Modified American Braille](https://en.wikipedia.org/wiki/American_Braille)
- **Inventor/Developer:** Joel W. Smith
- **Languages Supported:** US English (legacy)
- **Date:** 1878-1918
- **Key Characteristics:** Joel Smith, a teacher at the [Perkins Institution](https://en.wikipedia.org/wiki/Perkins_School_for_the_Blind), introduced Modified American Braille in 1878. Frequency-based dot assignments. Still used in some legacy publications
- **Status:** Legacy system

### [Iñupiaq Braille](https://en.wikipedia.org/wiki/I%C3%B1upiaq_Braille)
- **Languages Supported:** [Iñupiaq](https://en.wikipedia.org/wiki/Inupiat_language) (Alaska)
- **Key Characteristics:** Braille alphabet of the Inupiat language maintained by the Alaskan Department of Education. Based on French-ordered Braille. Print digraphs _ch_ and _sr_ are digraphs in braille as well. Uses special characters including _ñ_, _ŋ_, _ġ_, _ḳ_, _ṭ_, _ḷ_, _ł_, and _ł̣_. Punctuation is the same as in English Braille.
- **Status:** <span class="status-active">Active</span>

## Latin America

### [Spanish Braille (Latin American variants)](https://en.wikipedia.org/wiki/Spanish_Braille)
- **Languages Supported:** Spanish (Mexico, Central America, South America)
- **Key Characteristics:** Regional variations in some countries
- **Status:** <span class="status-active">Active</span>

### [Portuguese Braille (Brazilian)](https://en.wikipedia.org/wiki/Portuguese_Braille)
- **Languages Supported:** Portuguese (Brazil)
- **Key Characteristics:** PT-BR variant with slight differences from PT-PT
- **Status:** <span class="status-active">Active</span>

## Indigenous Languages

### [Inuktitut Braille](https://en.wikipedia.org/wiki/Inuktitut_Braille)
- **Languages Supported:** Inuktitut
- **Key Characteristics:** Adapted for Inuktitut syllabics; reassigned vowels
- **Status:** <span class="status-active">Active</span>

### [Guarani Braille](https://en.wikipedia.org/wiki/Guarani_Braille)
- **Languages Supported:** Guarani
- **Key Characteristics:** Based on French-ordered Braille
- **Status:** <span class="status-active">Active</span>

### [Hawaiian Braille](https://en.wikipedia.org/wiki/Hawaiian_Braille)
- **Languages Supported:** Hawaiian
- **Key Characteristics:** Based on French-ordered Braille
- **Status:** <span class="status-active">Active</span>

### [Iñupiaq Braille](https://en.wikipedia.org/wiki/I%C3%B1upiaq_Braille)
- **Languages Supported:** Iñupiaq
- **Key Characteristics:** Based on French-ordered Braille
- **Status:** <span class="status-active">Active</span>

### [Navajo Braille](https://en.wikipedia.org/wiki/Navajo_Braille)
- **Languages Supported:** Navajo
- **Key Characteristics:** Based on French-ordered Braille
- **Status:** <span class="status-active">Active</span>

### [Samoan Braille](https://en.wikipedia.org/wiki/Samoan_Braille)
- **Languages Supported:** Samoan
- **Key Characteristics:** Based on English/UEB Braille (not French-ordered)
- **Status:** <span class="status-active">Active</span>
- **Note:** Samoan Braille is a Pacific Island system; it is listed here for historical reasons but belongs in the Asian and Pacific section

### [Māori Braille](https://en.wikipedia.org/wiki/M%C4%81ori_Braille)
- **Languages Supported:** Māori
- **Key Characteristics:** Based on English/UEB Braille (not French-ordered)
- **Status:** <span class="status-active">Active</span>
- **Note:** Māori Braille is a New Zealand system; it is listed here for historical reasons but belongs in the Asian and Pacific section

## Notes

- Most Latin American countries use Spanish Braille
- Brazil uses Portuguese Braille (PT-BR variant)
- Some Caribbean countries use English or French Braille depending on colonial history

